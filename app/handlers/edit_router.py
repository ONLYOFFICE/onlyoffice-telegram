...
async def handle_edit_document_upload(message: Message, state: FSMContext, r: Redis):
    try:
        key = f"{message.chat.id}:{message.document.file_unique_id}"
        ...
        config_data = {
            ...
            "document_type": get_document_type(message.document.file_name),
            "file_type": get_file_type(message.document.file_name),
        ...
        web_app_url = f"https://t.me/{BOT_NAME}/{WEB_APP_NAME}?startapp={message.chat.id}_{message.document.file_unique_id}"
